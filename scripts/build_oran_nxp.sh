...
RUN_CMD="git clone --branch WRLINUX_10_18_BASE git://pek-git.wrs.com/git/wrlinux-x"
echo_cmd "Cloning wrlinux 1018 source from github:"
cd ${SRC_WRL_DIR}
${RUN_CMD}

RUN_CMD="./wrlinux-x/setup.sh --machines nxp-lx2xxx --layers meta-cloud-services --dl-layers"
...
if cd ${SCRIPTS_DIR} && git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
    ...
    cd ${SRC_ORAN_DIR}
    RUN_CMD="git clone --branch jackiehjm/master-nxp-20191107 http://stash.wrs.com/scm/~jhuang0/o-ran-pti-rtp.git rtp"
